...
const ScoreBoard = ({ innings, homeTeamScores, awayTeamScores }) => {
  ...
  return (
    <Box sx={{ border: '1px solid #ccc', padding: 2, borderRadius: 2, width: '100%', maxWidth: 600 }}>
      ...
      <Table>
        ...
        <TableBody>
          <TableRow>
            ...
            {Array.from({ length: totalInnings }, (_, i) => (
              <TableCell key={i} align="center">{awayTeamScores[i] !== undefined ? awayTeamScores[i] : ''}</TableCell>
            ))}
          </TableRow>
          <TableRow>
            <TableCell>홈팀</TableCell>
            {Array.from({ length: totalInnings }, (_, i) => (
              <TableCell key={i} align="center">{homeTeamScores[i] !== undefined ? homeTeamScores[i] : ''}</TableCell>
            ))}
          </TableRow>
        </TableBody>
      </Table>
